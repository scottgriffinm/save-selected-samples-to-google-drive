...
app.use(express.json());  // This line allows the server to parse JSON bodies
...
// Endpoint to get a batch of 3 random .wav files with "#" replaced by "sharp" in URLs
app.get("/api/samples", isAuthenticated, (req, res) => {
    ...
    fs.readdir(samplesDir, (err, files) => {
        ...
        // Filter for .wav files and replace "#" with "sharp" in URLs
        const wavFiles = files.filter(file => file.endsWith(".wav"));
        const selectedFiles = [];
        const fileMap = {}; // Map modified filenames back to originals

        while (selectedFiles.length < 3 && wavFiles.length > 0) {
            const randomIndex = Math.floor(Math.random() * wavFiles.length);
            const originalFile = wavFiles.splice(randomIndex, 1)[0];
            const modifiedFile = originalFile.replace(/#/g, "sharp"); // Replace # with "sharp"
            selectedFiles.push(modifiedFile);
            fileMap[modifiedFile] = originalFile; // Map modified filename to original
        }

        console.log("File map:", fileMap); // Log file map for debugging
        res.json({ files: selectedFiles, map: fileMap });
    });
});
...
app.post("/api/add-to-drive", isAuthenticated, async (req, res) => {
    const { fileName, map } = req.body;
    const originalFileName = map[fileName]; // Get the original filename from the map
